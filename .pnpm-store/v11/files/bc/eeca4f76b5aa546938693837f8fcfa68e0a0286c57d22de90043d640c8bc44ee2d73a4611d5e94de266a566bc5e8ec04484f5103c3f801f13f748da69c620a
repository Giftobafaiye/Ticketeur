...
import { ConfigRuntime } from "../schemas/build.js";
import { dedupFlags } from "./flags.js";
import { homedir } from "node:os";
export const DEFAULT_RUNTIME = "node";
export function isDeprecatedConfigRuntime(runtime) {
...
export function deprecatedRuntimeReplacement(runtime) {
    switch (runtime) {
...
export function resolveBuildRuntime(runtime) {
    ...
    if (!parsedRuntime.success) {
        ...
        throw new Error(`Unsupported runtime ${value} in trigger.config. Supported runtimes: ${ConfigRuntime.options.join(", ")}.`);
    }
