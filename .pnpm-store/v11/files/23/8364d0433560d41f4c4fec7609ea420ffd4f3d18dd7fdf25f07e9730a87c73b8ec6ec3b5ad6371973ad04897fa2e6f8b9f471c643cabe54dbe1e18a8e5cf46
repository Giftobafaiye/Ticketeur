...
exports.ensureAiSdkTelemetry = ensureAiSdkTelemetry;
/**
...
function isAutoRegisterDisabled() {
 ...
 * `@ai-sdk/otel`.
 */
function hasAiSdkOtelIntegration(OpenTelemetry) {
    const integrations = globalThis.AI_SDK_TELEMETRY_INTEGRATIONS;
    if (!Array.isArray(integrations)) {
        return false;
    }
    return integrations.some((integration) => (typeof OpenTelemetry === "function" && integration instanceof OpenTelemetry) ||
        integration?.constructor?.name === "OpenTelemetry");
}
//# sourceMappingURL=aiAutoTelemetry.js.map